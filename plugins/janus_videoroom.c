	...
	switch(media_type) {
		...
		case MEDIA_VIDEO:
			...
			if(vcodec == JANUS_VIDEOCODEC_VP8) {
				...
				" rtpjitterbuffer  name=rtpjitterbufferVideo ! rtpvp8depay name=rtpvp8depayVideo ! vp8dec ! queue ! "
				" videoconvert ! x264enc ! video/x-h264,profile=baseline ! h264parse name=h264parseVideo ! queue name=queueVideo ! "
