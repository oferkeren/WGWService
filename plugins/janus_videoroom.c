...
static guint32 janus_videoroom_rtp_forwarder_add_helper(janus_videoroom_publisher *p,
	...
	if(!is_data && srtp_suite > 0 && srtp_crypto != NULL) {
		...
		char srtp_id[MAX_STRING_LEN] = {0};
		g_snprintf(srtp_id, sizeof(srtp_id), "%s-%s-%"SCNu32"-%d", srtp_crypto, media, ssrc, pt);
...
static json_t *janus_videoroom_process_synchronous_request(janus_videoroom_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "create")) {
			...
			if(codec != NULL) {
				...
				while(codec != NULL) {
					...
					if(strlen(codec) == 0 || JANUS_VIDEOCODEC_NONE == janus_videocodec_from_name(codec)) {
						...
						error_code = JANUS_VIDEOROOM_ERROR_INVALID_ELEMENT;
						g_snprintf(error_cause, JANUS_ERROR_CAUSE_STRING_SIZE, "Invalid element (videocodec can only be or contain vp8, vp9, av1, h264 or h265)");
						goto prepare_response;
					}
					i++;
					codec = list[i];
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_videoroom *vr = value;
			JANUS_LOG(LOG_INFO, " ::: [%s][%s] %"SCNu32", max %d publishers, FIR frequency of %d seconds\n",
				vr->room_id_str, vr->room_name, vr->bitrate, vr->max_publishers, vr->fir_freq);
		}
		janus_mutex_unlock(&rooms_mutex);
		/* Send info back */
	...
	switch(media_type) {
		...
		case MEDIA_VIDEO:
			...
			if(vcodec == JANUS_VIDEOCODEC_VP8) {
				...
				" rtpjitterbuffer  name=rtpjitterbufferVideo mode=0 ! rtpvp8depay name=rtpvp8depayVideo ! vp8dec ! queue max-size-time=1000000 ! "
				" videoconvert ! x264enc ! video/x-h264,profile=baseline ! queue max-size-time=1000000 ! h264parse name=h264parseVideo ! queue name=queueVideo max-size-time=1000000 ! "
...
/*CARBYNE-RF-UPDATE-VIDEOROOM*/
			...
			if(!strcasecmp(request_text, "join")) {
				...
				if(publisher && (sdp_update || do_restart || do_update)) {
					...
					if(publisher->sdp != NULL) {
						char temp_error[JANUS_ERROR_CAUSE_STRING_SIZE];
		...
		if(!msg_sdp) {
			...
			if(session->participant_type != janus_videoroom_p_type_publisher) {
				...
				char error_str[JANUS_ERROR_CAUSE_STRING_SIZE];
